...
const localesDir = path.join(projectRoot, 'locales', 'calendar')
...
const run = async () => {
  ...
  if (localeFiles.length === 0) {
    throw new Error('No locale json files found in /locales/calendar')
  }
  ...
  const imports = localeFiles
    .map((file) => {
      ...
      return `import ${identifier} from "@/locales/calendar/${file}"`
    })
    .join('\n')
  ...
  console.log(
    `Generated ${path.relative(projectRoot, outputFile)} with ${localeFiles.length} locale(s).`,
  )
}
