...
export async function resolveHandle(
  ...
  const didDocRes = await fetch(
    `https://plc.directory/${encodeURIComponent(didData.did)}`,
    { cache: 'no-store' },
  )
  if (!didDocRes.ok) {
...
async function fetchWithDpopNonceRetry(params: {
  ...
  const hasDpop =
    !!params.accessToken && !!params.dpopPrivateKeyPem && !!params.dpopPublicJwk
...
export async function getProfile(
  ...
  actor: string,
  accessToken: string,
  dpop?: { privateKeyPem?: string; publicJwk?: DpopPublicJwk },
) {
...
export function profileAvatarBlobUrl(params: {
  pds: string
  did: string
  cid?: string
}) {
...
export async function putRecord(params: {
  ...
  const { pds, accessToken, dpopPrivateKeyPem, dpopPublicJwk, ...payload } =
    params
