...
export const PATCH = async function PATCH(
  ...
  if (status) {
    if (recurrenceId) {
      // Per-occurrence answer. The stamp may belong to any segment the token
      // addresses after a split, so find the grant that actually covers it —
      // and reject an uncovered stamp, or a participant could RSVP to, and
      // thereby confirm the existence of, occurrences they cannot see.
