...
export async function POST(request: NextRequest) {
  try {
    ...
    const encryptedPayload = hasPassword ? encryptWithKey(dataString, keyV3Password(password as string, id)) : { encryptedData: dataString, iv: "", authTag: "" };
    ...
    try {
      await client.query(
        ...
        `,
        [user.id, id, encryptedPayload.encryptedData, encryptedPayload.iv, encryptedPayload.authTag, new Date().toISOString(), hasPassword, burn, hasPassword ? 3 : 1],
      );
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      try {
        decryptedData = row.is_protected
          ? decryptWithKey(row.encrypted_data, row.iv, row.auth_tag, keyV3Password(password, id))
          : String(row.encrypted_data || "");
      } catch {
        await client.query("COMMIT");
