...
export function SignUpForm() {
  ...
  const verifyOtp = async () => {
    ...
    // Verified, so carry the return target through to sign-in rather than
    // dropping the user on a bare form -- they arrived from an app and are
    // trying to get back to it.
    //
    // The welcome email the calendar sent here is deliberately not sent: that
    // endpoint does not exist in the portal, so calling it would 404 silently.
    // Whether the portal owns that email is a product decision, not something
    // to inherit by accident.
    const params = new URLSearchParams(window.location.search)
    const requested = params.get(RETURN_TO_PARAM)
    router.push(
      requested
        ? `/sign-in?${RETURN_TO_PARAM}=${encodeURIComponent(requested)}`
        : '/sign-in',
    )
    router.refresh()
  }
