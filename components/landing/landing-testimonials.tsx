...
export function LandingTestimonials() {
  return (
    <section className="flex min-h-screen items-center border-b border-white/10 py-12">
      <div className="w-full">
        <h2 className="landing-title-reveal text-3xl font-semibold text-white md:text-5xl">Why users choose One Calendar</h2>
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          {feedback.map((item) => (
            <article key={item.title} className="rounded-2xl border border-white/10 bg-[var(--landing-panel)] p-6">
              <h3 className="landing-title-reveal text-xl font-medium text-white md:text-2xl">{item.title}</h3>
