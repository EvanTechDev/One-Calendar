...
export function DialogTrigger({
  ...
  return (
    <DialogPrimitive.Trigger
      ...
      render={asChild && isValidElement(children) ? Children.only(children) : undefined}
      {...props}
    >
      {children}
    </DialogPrimitive.Trigger>
...
export function DialogClose({
  ...
  children,
  ...props
}: DialogPrimitive.Close.Props & { asChild?: boolean }): React.ReactElement {
  return (
...
export function DialogHeader({
  ...
  const defaultProps = {
    className: cn(
      "flex flex-col gap-2 in-[[data-slot=dialog-popup]:has([data-slot=dialog-panel])]:pb-3 max-sm:pb-4",
      className,
    ),
...
export function DialogFooter({
  ...
  const defaultProps = {
    className: cn(
      "flex flex-col-reverse gap-2 sm:flex-row sm:justify-end sm:rounded-b-[calc(var(--radius-2xl)-1px)]",
      variant === "default" && "border-t bg-muted/72 py-4",
      variant === "bare" && "in-[[data-slot=dialog-popup]:has([data-slot=dialog-panel])]:pt-3 pt-4",
      className,
    ),
...
export function DialogPanel({
  ...
  const defaultProps = {
    className: cn(
      "in-[[data-slot=dialog-popup]:has([data-slot=dialog-header])]:pt-1 in-[[data-slot=dialog-popup]:has([data-slot=dialog-footer]:not(.border-t))]:pb-1",
      className,
    ),
