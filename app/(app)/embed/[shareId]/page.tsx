...
export default function EmbedEventPage({ params }: { params: Promise<{ shareId: string }> }) {
  ...
  if (error || !event) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-white text-black dark:bg-neutral-900 dark:text-neutral-100">
        <Card className="max-w-md w-full overflow-hidden bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700">
          <CardContent className="p-6 text-center">
            ...
            <CardDescription className="text-gray-600 dark:text-gray-300">
              {language === "zh" ? "无法加载共享的日历事件。该链接可能已过期或无效。" : "Unable to load the shared calendar event. The link may be expired or invalid."}
            </CardDescription>
          </CardContent>
        </Card>
      </div>
  ...
  return (
    <div className="p-4 min-h-screen bg-white text-black dark:bg-neutral-900 dark:text-neutral-100">
      <Card className="max-w-md w-full overflow-hidden bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700">
        <div className="relative">
          ...
          <CardContent className="p-6">
            <div className="flex justify-between items-start mb-4">
              ...
              <Badge variant="outline" className="flex items-center gap-1 border-neutral-300 dark:border-neutral-600">
                <Clock className="h-3 w-3" />
                <span>{durationText}</span>
              </Badge>
            </div>
            <Card className="bg-muted dark:bg-neutral-700/50 mb-6 border-none shadow-none">
              <CardContent className="p-4 flex items-start">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground dark:text-neutral-300">{language === "zh" ? "至" : "to"} {formatDateWithTimezone(event.endDate)}</p>
                </div>
              </CardContent>
            </Card>
            <div className="space-y-5">
              {event.location && (
                <div className="flex items-start">
                  <MapPin className="h-5 w-5 mr-3 mt-0.5 text-muted-foreground dark:text-neutral-400" />
                  <p>{event.location}</p>
                </div>
              )}
              {event.participants?.length > 0 && (
                <div className="flex items-start">
                  <Users className="h-5 w-5 mr-3 mt-0.5 text-muted-foreground dark:text-neutral-400" />
                  <p>{event.participants.join(", ")}</p>
                </div>
              )}
              {event.notification > 0 && (
                <div className="flex items-start">
                  <Bell className="h-5 w-5 mr-3 mt-0.5 text-muted-foreground dark:text-neutral-400" />
                  <p>{language === "zh" ? `提前 ${event.notification} 分钟提醒` : `${event.notification} minutes before`}</p>
                </div>
              )}
              {event.description && (
                <div className="flex items-start">
                  <AlignLeft className="h-5 w-5 mr-3 mt-0.5 text-muted-foreground dark:text-neutral-400" />
